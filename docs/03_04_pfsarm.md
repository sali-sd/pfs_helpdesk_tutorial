...
Example from proposal `S25A-000QF`, visit 137292 on the Science Platform (blue `b`, red `r`, and IR `n` arms across 4 spectrograph modules):

```
/shared/pfs/programs/S25A-000QF/2d/run26_June2026/pfsArm/20260111/137292/
    pfsArm_PFS_137292_b1_run26_June2026.fits
    pfsArm_PFS_137292_b2_run26_June2026.fits
    pfsArm_PFS_137292_b3_run26_June2026.fits
    pfsArm_PFS_137292_b4_run26_June2026.fits
    pfsArm_PFS_137292_r1_run26_June2026.fits
    ...
    pfsArm_PFS_137292_n4_run26_June2026.fits
```
...
The following plots the `pfsArm` spectrum of a single object from a single visit. Each arm is fetched and plotted individually with its own color. Instantiate `Butler` by providing the datastore `repo` and `collections`. Specify a `visit` and `objid` to plot that object directly, or use `browse_index` to step through all science objects in a visit sorted by `objId`. The specific arms to be plotted can be selected and the spectra can be smoothed using a median filter if desired.
...
def plot_pfsarm(repo, collections, visit, objid, browse_index, MEDIAN_FILTER_SIZE, arms):
    ...
    plt.savefig(f'pfsArm_{collections}_{objid}.png', dpi=150, bbox_inches='tight')
    plt.show()
...
![pfsArm spectrum for objId 89100543080260387](figures/pfsArm_run26_June2026_89100543080260387.png)
