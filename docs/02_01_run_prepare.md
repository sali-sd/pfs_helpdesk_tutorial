...
The first step is to make a copy of the default `butler.yaml`:

```bash
cp $OBS_PFS_DIR/gen3/butler.yaml $WORKDIR/$(whoami)/data/butler.yaml
```

This file specifies two configuration files that are important for the Gen3 middleware. `butler.yaml` contains the configuration for the data `butler`, specifying the registry and how the pipeline will read and write the various data products.
...
```bash
export DATASTORE="$WORKDIR/$(whoami)/data/datastore"
butler create $DATASTORE --seed-config $WORKDIR/$(whoami)/data/butler.yaml --dimension-config $OBS_PFS_DIR/gen3/dimensions.yaml --override
```
...
butler collection-chain $DATASTORE PFS/calib PFS/calib/curated/19700101T000000Z,PFS/calib/curated/20200101T000000Z,PFS/calib/curated/20230401T000000Z,PFS/calib/curated/20230701T000000Z,PFS/calib/curated/20240815T000000Z,PFS/calib/unbounded
```
