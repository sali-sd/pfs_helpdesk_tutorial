...
`pfsSingle` is the **flux-calibrated**, arm-merged spectrum for a single object
from a single visit. It is a per-object file (one file per science target per visit).
The format is identical to `pfsObject` (see below).

---

## pfsCalibrated

`pfsCalibrated` is a **visit-level bundle** of flux-calibrated spectra — essentially
a collection of `pfsSingle` spectra for all objects in a single visit, stored in one file.
The format is identical to `pfsCoadd` (see below).
...
`pfsObject` is the **coadded, flux-calibrated spectrum** for a single object
across multiple visits. The filename encodes the number of visits (`nVisit`, modulo 1000)
and a 63-bit SHA-1 hash of the contributing visits (`pfsVisitHash`) to uniquely
identify the combination. Files are organised on disk as `catId/tract/patch/pfsObject-*.fits`.
