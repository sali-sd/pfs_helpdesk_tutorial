...
``` bash
# Setting environment for $WORKDIR/$(whoami)/bin/user-yum.sh/root
ROOT_D="$WORKDIR/$(whoami)/bin/user-yum.sh/root"
...
```bash
cd $WORKDIR/$(whoami)/bin/user-yum.sh/user-yum.sh/
...
cd git-lfs-3.5.1
PREFIX=$WORKDIR/$(whoami) ./install.sh
```
